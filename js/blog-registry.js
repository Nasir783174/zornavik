const BLOG_REGISTRY = [
  {
    title:    "Best Upright Vacuum Cleaners of 2026",
    slug:     "posts/best-bissell-vacuum-2026.html",
    date:     "2026-04-13",
    excerpt:  "Choosing the right upright vacuum in 2026 feels harder than it should be. I tested 5 top models — Shark Stratos, Dyson Ball Animal 3, Shark Navigator, Kenmore Intuition, and Bissell Pet Hair Eraser — so you can find the one that fits your home.",
    readTime: 14,
    category: "vacuums",
    image:    "../images/bissell vacuum/bissell vacuum cleaner.webp",
  },

  {
    title:    "Best Lightweight Vacuum in 2026",
    slug:     "posts/best-lightweight-vacuum-2026.html",
    date:     "2026-04-27",
    excerpt:  "Not all lightweight vacuums are truly light where it matters. I broke down the top 5 models — Dyson V15, Shark Stratos Cordless, Dyson V12, Levoit LVAC-200, and Tineco Pure One Station — so you can choose based on real use, not marketing claims.",
    readTime: 16,
    category: "cordless-stick-vacuums",
    image:    "",
  },
  {
    title:    "Bissell Vacuum Cleaner in 2026",
    slug:     "posts/bissell-vacuum-cleaner-in-2026.html",
    date:     "2026-04-26",
    excerpt:  "Bissell vacuum cleaners continue to hold a strong place in modern homes because they focus on real cleaning problems people face every day. From pet hair on sofas to muddy footprints on hard floors...",
    readTime: 8,
    category: "vacuums",
    image:    "../images/cordless-vacuum-cleaners/cordless-vacuum-cleaners.webp",
  },
  {
    title:    "Best Cordless Vacuum Cleaner in 2026",
    slug:     "posts/best-cordless-vacuum-cleaners-2026.html",
    date:     "2026-04-26",
    excerpt:  "Bissell vacuum cleaners continue to hold a strong place in modern homes because they focus on real cleaning problems people face every day. From pet hair on sofas to muddy footprints on hard floors...",
    readTime: 8,
    category: "cordless-stick-vacuums",
    image:    "",
  },
];

/*
  CATEGORY SLUGS (for reference when adding new posts):
  ─────────────────────────────────────────────────────
  "robot-vacuums"          → categories/robot-vacuums.html
  "vacuums"                → categories/vacuums.html
  "cordless-stick-vacuums" → categories/cordless-stick-vacuums.html
*/
